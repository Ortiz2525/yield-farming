...
import { Attribute, Royalty, Metadata, SCBehavior } from '../structs/DynamicMetadataStructs.sol';

abstract contract GMDynamicSC {
  using Strings for uint256;
  using DynamicMetadata for Attribute[];
  using DynamicMetadata for Metadata;
  ...
  function _getDynamicTokenURI(uint256 tokenId) internal view returns (string memory) {
    ...
    if (attributes.length != 0) {
      ...
      Metadata memory metadata = Metadata(_metadataDescription, metadataImageURL, metadataName, attributes, royalty);
  ...
  function _dynamicCanTokenBeTransferred(address from, uint256 tokenId) private view returns (bool) {
    return IGMTransferController(_transferControllerSC).canTokenBeTransferred(address(this), from, tokenId);
  }
  ...
  function _defaultCanTokenBeTransferred(address, uint256) private pure returns (bool) {
    return true;
  }
